...
namespace admin
{
    public class Admin
    {
        ...
        public void StartGame(Socket socket)
        {
            ...
            playerNumber = GetPlayerNumber(socket);
            gameStart = true;

            SendMessage("playerCardSwap");
            playerSwap = GetPlayerSwap(socket);
            swapMade = true;

            dynamic[] args = { AdminNumber };
            SendMessage("IsPlayerWinner", args);
            playerWinner = GetIsPlayerWinner(socket);
        ...
        bool GetIsPlayerWinner(Socket socket)
        {
        ...
        int GetPlayerNumber(Socket socket)
        {
            if (gameStart)
            {
        ...
        bool GetPlayerSwap(Socket socket)
        {
            if (swapMade)
            {
